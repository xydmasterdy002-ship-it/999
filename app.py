# ========================= FINAL STABLE VERSION (All Models Integrated) =========================
import os
import json
import torch
...
MODEL_FILES = {
    ...
    "EfficientNet-B0": {
        "file_id": "10P8pjuyQqrXiCZJhujOKesS3263-65iC",
        "path": "EfficientNet-B0_imagenet100.pth"
    },
    "VGG-16": {
        "file_id": "1LE5ghQK-uDipL1uh35LazL1j2ivVgRnh",
        "path": "VGG-16_imagenet100.pth"
    },
    "ViT": {
        "file_id": "1KEZOMHPp0KTXLSMP1jiYVOh0-cv8hMxI",
        "path": "ViT_imagenet100.pth"
    }
}
...
class ResNet101Custom(nn.Module):
    def __init__(self, num_classes=100, include_top=True):
        super().__init__()
...
def load_model(model_name):
    ...
    if model_name=="MobileNet-V1":
        ...
        model = ResNet101Custom(num_classes=num_classes)
    elif model_name=="EfficientNet-B0":
        model = EfficientNetB0Custom(num_classes=num_classes)
    elif model_name=="VGG-16":
        model = VGG16Custom(num_classes=num_classes)
    elif model_name=="ViT":
        model = ViTB16Custom(num_classes=num_classes)

    state_dict = torch.load(info["path"], map_location=device)
    new_state_dict = {k.replace("module.",""):v for k,v in state_dict.items()}
    strict_flag = model_name in ["EfficientNet-B0","VGG-16","ViT"] and False or True
    model.load_state_dict(new_state_dict, strict=strict_flag)
...
if "history" not in st.session_state:
    st.session_state.history = pd.DataFrame(columns=[
        ...
        "1st class","1st P(%)","2nd class","2nd P(%)","3rd class","3rd P(%)","4th class","4th P(%)","5th class","5th P(%)"
    ])
