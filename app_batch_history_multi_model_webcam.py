...
class GoogLeNetCustom(nn.Module):
    def __init__(self, num_classes=100):
        ...
        base.aux1.fc2 = nn.Linear(base.aux1.fc2.in_features, num_classes)
        base.aux2.fc2 = nn.Linear(base.aux2.fc2.in_features, num_classes)

        self.base = base   # ✅ 不用 model 這個名字
...
def load_model(model_name):
    ...
    if missing:
        st.warning(f"Missing keys: {missing[:5]}")
    if unexpected:
        st.warning(f"Unexpected keys: {unexpected[:5]}")
