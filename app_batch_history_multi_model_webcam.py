...
class GoogLeNetCustom(nn.Module):
    def __init__(self, num_classes=100):
        super().__init__()
        self.model = models.googlenet(weights=models.GoogLeNet_Weights.DEFAULT, aux_logits=False)
...
for fname,img in images:
    ...
    for i,(eng,zh,score) in enumerate(results):
        row[f"{i+1}st class" if i==0 else f"{i+1}nd class" if i==1 else f"{i+1}rd class" if i==2 else f"{i+1}th class"] = f"{eng}({zh})"
        row[f"{i+1}st P(%)" if i==0 else f"{i+1}nd P(%)" if i==1 else f"{i+1}rd P(%)" if i==2 else f"{i+1}th P(%)"] = round(score,2)
