# ========================= FINAL STABLE VERSION (按鈕觸發攝像頭 + fallback) =========================
import os
import json
import torch
...
class GoogLeNetRobust(nn.Module):
    def __init__(self, num_classes=100, pretrained=False, use_aux=True, device=None):
        ...
        self.model = models.googlenet(weights=None if not pretrained else models.GoogLeNet_Weights.DEFAULT, aux_logits=use_aux)
...
st.info("如要使用攝像頭拍照，請先點擊下方按鈕啟動攝像頭")
use_cam = st.button("啟動攝像頭")
